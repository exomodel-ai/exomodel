...
solution_description: Phase 1: Discovery & Stakeholder Alignment (Weeks 1-3): Deep-dive workshops with Marketing, Logistics, and IT heads to map current pain points. Deliverable: Current State AI Maturity Assessment.Phase 2: Technical & Data Audit (Weeks 4-5): Evaluation of existing data quality, cloud infrastructure, and legacy retail systems to determine "AI Readiness." Deliverable: Data Gap Analysis & Infrastructure Report.Phase 3: Opportunity Matrix & Prioritization (Weeks 6-7): Scoring of 10+ potential AI use cases (e.g., Dynamic Pricing, Churn Prediction) based on Business Impact vs. Implementation Effort. Deliverable: Prioritized AI Use-Case Matrix (ROI vs. Feasibility).Phase 4: Final 12-Month Roadmap & Governance (Week 8): Creation of a sequenced implementation plan including estimated costs, team requirements, and vendor recommendations. Deliverable: Executive AI Strategy Masterplan & Implementation Timeline.
duration: 2
...
def test_to_ui_html_format():
    proposal = Proposal()
    proposal.create_example()
    output = proposal.to_ui(format="html")
    assert "<b>" in output
    assert "**" not in output
    assert output.startswith("<b>PROPOSAL</b>")
    assert "RetailFlow" in output

def test_to_ui_markdown_format():
    proposal = Proposal()
    proposal.create_example()
    output = proposal.to_ui(format="markdown")
    assert "**" in output
    assert "*" in output
    assert "<b>" not in output
    assert "<i>" not in output
    assert output.startswith("**PROPOSAL**")

def test_to_ui_plain_format():
    proposal = Proposal()
    proposal.create_example()
    output = proposal.to_ui(format="plain")
    assert "<b>" not in output
    assert "<i>" not in output
    assert "**" not in output
    assert "PROPOSAL" in output

def test_to_ui_default_is_html():
    proposal = Proposal()
    proposal.create_example()
    assert proposal.to_ui() == proposal.to_ui(format="html")

def test_to_ui_empty_fields_html():
    proposal = Proposal()
    output = proposal.to_ui(format="html")
    assert "<i>Not provided</i>" in output

def test_to_ui_empty_fields_markdown():
    proposal = Proposal()
    output = proposal.to_ui(format="markdown")
    assert "*Not provided*" in output

def test_to_ui_empty_fields_plain():
    proposal = Proposal()
    output = proposal.to_ui(format="plain")
    assert "Not provided" in output
    assert "<i>" not in output
...
def test_usage_tracking():
    proposal = Proposal()
    proposal.create_example()
    proposal.run_object_prompt("What's the name of the client? Return only the name.")
    usage = proposal._exo_agent.get_usage()
...
def test_usage_reset():
    ...
    proposal.create_example()
    proposal.run_object_prompt("What's the name of the client? Return only the name.")
    proposal._exo_agent.reset_usage()
    usage = proposal._exo_agent.get_usage()
